...
def main():
   ...
   ###OPTIONAL, FOR FEATURES REPRESENTATION
    ...
    data=initGraph()
    if length1 / data['width'] > scale:
       estscale = int(length1 / data['width']) + 1
       sys.exit("\n\n! The sequence in %s is predicted to extend beyond the plot width, you must increase the scale to at least %i --fatal." % (txt_file1, estscale))
    if length2 / data['width'] > scale:
       estscale = int(length2 / data['width']) + 1
       sys.exit("\n\n! The sequence in %s is predicted to extend beyond the plot width, you must increase the scale to at least %i --fatal." % (txt_file2, estscale))
    if length3 / data['width'] > scale:
       estscale = int(length3 / data['width']) + 1
       sys.exit("\n\n! The sequence in %s is predicted to extend beyond the plot width, you must increase the scale to at least %i --fatal." % (txt_file3, estscale))
