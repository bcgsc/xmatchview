...
# Rene L Warren 2005,2017,2018,2019
...
def readGFF(file,scale):
   ...
   with open(file) as fd:
      ...
      for row in rd:
         ...
         if not feature.has_key(id):
            feature[id] = {}
         if not feature[id].has_key(start):
            feature[id][start] = {}
         if not feature[id][start].has_key('end'):
            feature[id][start]['end'] = "" 
         if not feature[id][start].has_key('color'):
   ...
   for line in xmatch_obj:
      ...
      if rm != None:
         ...
         if (primary_match == secondary_match) and (startSecondMatch == startFirstMatch) and (endSecondMatch == endFirstMatch):
            ...
            if query.has_key(primary_match) and reference.has_key(secondary_match):        #RLW after reading FASTA for each
               ...
               if not nocdt[primary_match][secondary_match].has_key(startFirstMatchS):
                  nocdt[primary_match][secondary_match][startFirstMatchS]={}
               if not nocdt[primary_match][secondary_match][startFirstMatchS].has_key(endFirstMatchS):
                  nocdt[primary_match][secondary_match][startFirstMatchS][endFirstMatchS]={}
               if not nocdt[primary_match][secondary_match][startFirstMatchS][endFirstMatchS].has_key(startSecondMatchS):
                  nocdt[primary_match][secondary_match][startFirstMatchS][endFirstMatchS][startSecondMatchS]={}
               if not nocdt[primary_match][secondary_match][startFirstMatchS][endFirstMatchS][startSecondMatchS].has_key(endSecondMatchS):
         ...
         if percentMis > mismatch:
            ...
            if query.has_key(primary_match) and reference.has_key(secondary_match):
   ...
   for pos in pos_range:
      ...
      for query in nocdt:
         for comparison in nocdt[query]:
            start1_dict=nocdt[query][comparison].keys()
            start1_dict.sort()
            for start1 in start1_dict:
               end1_dict=nocdt[query][comparison][start1].keys()
               end1_dict.sort()
               for end1 in end1_dict:
                  start2_dict=nocdt[query][comparison][start1][end1].keys()
                  start2_dict.sort()
                  for start2 in start2_dict:
                     end2_dict=nocdt[query][comparison][start1][end1][start2].keys()
                     end2_dict.sort()
   ...
   for line in file_obj:
      ...
      if head_match != None:
          if (head_match != previous_contig and previous_contig != None):
             ...
             if not L1.has_key(previous_contig):#RLW
...
def drawRelationship(reference_list, order_ref, query_list, order_qry, match_list, scale, mismatch, block_length, alignment_file, freq, reflength, leap, format, formatdict, protein, alpha, refgff, qrygff, qrylength, fontpath):
      ...
      for ref in order_ref:      #RLW
         ...
         if reference_list.has_key(ref): #RLW
      ...
      ###DRAW histo
      x_range=range(data['x'], last_coord, 100)
      ### draw kbp scale
      if reflength >= 10000:
      ...
      grid_range=range(data['mis_bar'], data['ref_y']-30, 20)
      ...
      for ref in order_ref:
         ...
         if reference_list[ref].has_key('npos'):
         ...
         if reference_list[ref].has_key('zpos'):        ### draw Zs
      ...
      for qry in order_qry:
         ...
         if qry not in order_ref:  ### only draw if query not same as ref
            ...
            if query_list[qry].has_key('npos'):            ### draw Ns
            ...
            if query_list[qry].has_key('zpos'):            ### draw Zs
...
def main():
   ...
   if (alignment_file == None or reference_file == None or query_file == None or mismatch == 0 or block_length == 0 or scale ==0 or leap == 0):
      print "Usage: %s v1.2.0" % (sys.argv[0:])
